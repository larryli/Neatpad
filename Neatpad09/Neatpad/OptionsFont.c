                           ...
                           DWORD FontType, LPARAM lParam)
{
    ...
    if (fTrueType) {
        for (i = 0; i < (int)(sizeof(ttsizes) / sizeof(ttsizes[0])); i++) {
            _stprintf(ach, sizeof(ach) / sizeof(TCHAR), _T("%d"), ttsizes[i]);
        ...
        _stprintf(ach, sizeof(ach) / sizeof(TCHAR), _T("%d"), size);
...
BOOL InitFontOptionsDlg(HWND hwnd)
{
    ...
    //	Select
    //
    _stprintf(ach, sizeof(ach) / sizeof(TCHAR), _T("%d"), g_nFontSize);
