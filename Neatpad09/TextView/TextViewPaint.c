...
int TextView__PaintMargin(TEXTVIEW *ptv, HDC hdc, ULONG nLineNo, RECT *margin)
{
    ...
    if (TextView__CheckStyle(ptv, TXS_LINENUMBERS)) {
        ...
        rect.right += 1;
        // PaintRectPoint(hdc, &rect, MixRGB(GetSysColor(COLOR_3DFACE),
        // 0xffffff));
        ...
        /*	rect.left   = rect.right;
            rect.right += 5;
            PaintRectPoint(hdc, &rect, TextView__GetColour(ptv,
           TXC_BACKGROUND));*/
...
void TextView__PaintText(TEXTVIEW *ptv, HDC hdc, ULONG nLineNo, RECT *rect)
{
    ...
    //
    while ((len = TextIterator__gettext(itor, buff, TEXTBUFSIZE)) > 0) {
        //
        //	Apply text attributes -
        //	i.e. syntax highlighting, mouse selection colours etc.
        //
        len = TextView__ApplyTextAttributes(ptv, nLineNo, off_chars, &colno,
                                            buff, len, attr);
                                  ...
                                  ATTR *attr)
{
    ...
    //
    return TextView__StripCRLF(ptv, szText, nTextLen, FALSE);
}
                          ...
                          TCHAR *szText, int nLen, int nTabOrigin, ATTR *attr)
{
    ...
    /*if(ptv->m_fTransparent && attr->bg == TextView__GetColour(ptv,
    TXC_BACKGROUND))
    {
...
//
int TextView__StripCRLF(TEXTVIEW *ptv, TCHAR *szText, int nLength, BOOL fAllow)
{
    if (nLength >= 2) {
    ...
    if (nLength >= 1) {
        if (szText[nLength - 1] == '\r' && (ptv->m_nCRLFMode & TXL_CR) ||
            szText[nLength - 1] == '\n' && (ptv->m_nCRLFMode & TXL_LF)) {
