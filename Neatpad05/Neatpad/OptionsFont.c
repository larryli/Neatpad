...
BOOL FontCombo_DrawItem(HWND hwnd, DRAWITEMSTRUCT *dis)
{
    ...
    // draw the text
    ExtTextOut(dis->hDC, xpos, ypos, ETO_OPAQUE, &dis->rcItem, szText,
               strlen(szText), 0);
                          ...
                          BOOL fSelectImage)
{
    ...
    ExtTextOut(dis->hDC, xpos, ypos, ETO_OPAQUE, &rect, szText, lstrlen(szText),
               0);
...
void UpdatePreviewPane(HWND hwnd)
{
    ...
    g_hPreviewFont =
        EasyCreateFont(size, IsDlgButtonChecked(hwnd, IDC_BOLD), szFaceName);
