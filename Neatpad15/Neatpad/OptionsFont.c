...
DWORD GetCurrentComboData(HWND hwnd, UINT uCtrl)
{
    ...
    return SendDlgItemMessage(hwnd, uCtrl, CB_GETITEMDATA, idx == -1 ? 0 : idx,
                              0);
}
...
DWORD GetCurrentListData(HWND hwnd, UINT uCtrl)
{
    ...
    return SendDlgItemMessage(hwnd, uCtrl, LB_GETITEMDATA, idx == -1 ? 0 : idx,
                              0);
}
...
//
int CALLBACK EnumFontNames(ENUMLOGFONTEX *lpelfe, NEWTEXTMETRICEX *lpntme,
                           DWORD FontType, LPARAM lParam)
{
    ...
    if (SendMessage(hwndCombo, CB_FINDSTRING, 0, (LPARAM)pszName) == CB_ERR) {
        ...
        fFixed =
            (lpelfe->elfLogFont.lfPitchAndFamily & FIXED_PITCH) ? TRUE : FALSE;
        fTrueType = (lpelfe->elfLogFont.lfOutPrecision == OUT_STROKE_PRECIS)
                        ? TRUE
                        : FALSE;
        fTrueType = (lpntme->ntmTm.ntmFlags & NTM_TT_OPENTYPE) ? 2 : fTrueType;

        // store this information in the list-item's userdata area
        SendMessage(hwndCombo, CB_SETITEMDATA, idx,
...
void FillFontComboList(HWND hwndCombo)
{
    ...
    EnumFontFamiliesEx(hdc, &lf, (FONTENUMPROC)EnumFontNames, (LPARAM)hwndCombo,
                       0);
...
void FillFileTypeList(HWND hwnd)
{
    ...
    for(i = 0; lookup[i]; i++)
    {
        ...
        hImgList = (HIMAGELIST)SHGetFileInfo(lookup[i], FILE_ATTRIBUTE_NORMAL,
&shfi, sizeof(shfi), SHGFI_TYPENAME|SHGFI_ICON
...
BOOL FontCombo_DrawItem(HWND hwnd, DRAWITEMSTRUCT *dis)
{
    ...
    ypos = dis->rcItem.top +
           (dis->rcItem.bottom - dis->rcItem.top - tm.tmHeight) / 2;
    xpos = dis->rcItem.left + 20;

    // draw the text
    ExtTextOut(dis->hDC, xpos, ypos, ETO_CLIPPED | ETO_OPAQUE, &dis->rcItem,
               szText, _tcslen(szText), 0);

    // draw a 'TT' icon if the font is TRUETYPE
    if (fTrueType)
        DrawIconEx(dis->hDC, dis->rcItem.left + 2, dis->rcItem.top, g_hIcon2,
                   16, 16, 0, 0, DI_NORMAL);
    // else if(fTrueType == 2)
    //	DrawIconEx(dis->hDC, dis->rcItem.left+2, dis->rcItem.top, g_hIcon3,16,
    // 16, 0, 0, DI_NORMAL);
...
//
BOOL ColourCombo_DrawItem(HWND hwnd, UINT uCtrlId, DRAWITEMSTRUCT *dis,
                          BOOL fSelectImage)
{
    ...
    ExtTextOut(dis->hDC, xpos, ypos, ETO_CLIPPED | ETO_OPAQUE, &rect, szText,
               lstrlen(szText), 0);
    ...
    if (dis->itemState & ODS_DISABLED)
        PaintFrameRect(dis->hDC, &rect, GetSysColor(COLOR_3DSHADOW),
                       GetSysColor(COLOR_3DFACE));
    else
        PaintFrameRect(dis->hDC, &rect, RGB(0, 0, 0),
                       REALIZE_SYSCOL(dis->itemData));

    return TRUE;
}

int CALLBACK EnumFontSizes(ENUMLOGFONTEX *lpelfe, NEWTEXTMETRICEX *lpntme,
                           DWORD FontType, LPARAM lParam)
{
    static int ttsizes[] = {8,  9,  10, 11, 12, 14, 16, 18,
                            20, 22, 24, 26, 28, 36, 48, 72};
    TCHAR ach[100];

    BOOL fTrueType =
        (lpelfe->elfLogFont.lfOutPrecision == OUT_STROKE_PRECIS) ? TRUE : FALSE;

    HWND hwndCombo = (HWND)lParam;
    int i, count, idx;

    if (fTrueType) {
        for (i = 0; i < (int)(sizeof(ttsizes) / sizeof(ttsizes[0])); i++) {
...
void InitSizeList(HWND hwnd)
{
    ...
    lf.lfPitchAndFamily = 0;
    SendDlgItemMessage(hwnd, IDC_FONTLIST, CB_GETLBTEXT, item,
    ...
    EnumFontFamiliesEx(hdc, &lf, (FONTENUMPROC)EnumFontSizes,
                       (LPARAM)GetDlgItem(hwnd, IDC_SIZELIST), 0);
...
LONG CALLBACK PreviewWndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_PAINT:
        ...
        DrawText(ps.hdc, _T("Sample Text"), -1, &rect,
...
void AddColourListItem(HWND hwnd, UINT uItem, int fgIdx, int bgIdx,
                       TCHAR *szName)
{
...
void UpdatePreviewPane(HWND hwnd)
{
    ...
    SendDlgItemMessage(hwnd, IDC_FONTLIST, CB_GETLBTEXT, idx,
                       (LPARAM)szFaceName);
    ...
    g_hPreviewFont = EasyCreateFont(size, IsDlgButtonChecked(hwnd, IDC_BOLD),
                                    g_tempFontSmoothing, szFaceName);
    ...
    // g_crPreviewFG = SendDlgItemMessage(hwnd, IDC_FGCOLCOMBO, CB_GETITEMDATA,
    // idx, 0);
    ...
    // g_crPreviewBG = SendDlgItemMessage(hwnd, IDC_BGCOLCOMBO, CB_GETITEMDATA,
    // idx, 0);
...
void SelectColorInList(HWND hwnd, UINT uComboIdx, short itemIdx)
{
    ...
    // normal colour
        ...
        if (i == NUM_DEFAULT_COLOURS) {
            ...
            SendMessage(hwndCombo, CB_SETITEMDATA, i,
                        g_rgbTempColourList[itemIdx]);
            SendMessage(hwndCombo, CB_SETCURSEL, i, 0);
        }
    }
}
...
BOOL InitFontOptionsDlg(HWND hwnd)
{
    ...
    memcpy(g_rgbTempColourList, g_rgbColourList,
           sizeof(COLORREF) * TXC_MAX_COLOURS);

    //
    //	Load the TrueType icon for the font-list
    //
    g_hIcon2 = LoadImage(GetModuleHandle(0), MAKEINTRESOURCE(IDI_ICON2),
                         IMAGE_ICON, 16, 16, 0);
    g_hIcon3 = LoadImage(GetModuleHandle(0), MAKEINTRESOURCE(IDI_ICON3),
                         IMAGE_ICON, 16, 16, 0);
    ...
    g_hPreviewFont = EasyCreateFont(g_nFontSize, g_fFontBold,
                                    g_tempFontSmoothing, g_szFontName);
    ...
    oldPreviewProc = (WNDPROC)SetWindowLongPtr(hwndPreview, GWLP_WNDPROC,
                                               (LPARAM)PreviewWndProc);

    //	g_rgbAutoColourList[TXC_LONGLINE]	= MixRGB(GetSysColor(COLOR_3DFACE),
    // GetSysColor(COLOR_WINDOW)); 	g_rgbAutoColourList[TXC_LINENUMBER]	=
    // MixRGB(GetSysColor(COLOR_3DFACE), GetSysColor(COLOR_WINDOW));
    //	g_rgbAutoColourList[TXC_LINENUMBER] =
    // MixRGB(g_rgbAutoColourList[TXC_LINENUMBER], GetSysColor(COLOR_WINDOW));
    ...
    AddColourListItem(hwnd, IDC_LIST1, TXC_FOREGROUND, TXC_BACKGROUND,
                      _T("Text"));
    AddColourListItem(hwnd, IDC_LIST1, TXC_HIGHLIGHTTEXT, TXC_HIGHLIGHT,
                      _T("Selected Text"));
    AddColourListItem(hwnd, IDC_LIST1, TXC_HIGHLIGHTTEXT2, TXC_HIGHLIGHT2,
                      _T("Inactive Selection"));
    AddColourListItem(hwnd, IDC_LIST1, TXC_SELMARGIN1, TXC_SELMARGIN2,
                      _T("Left Margin"));
    AddColourListItem(hwnd, IDC_LIST1, TXC_LINENUMBERTEXT, TXC_LINENUMBER,
                      _T("Line Numbers"));
    AddColourListItem(hwnd, IDC_LIST1, -1, TXC_LONGLINE, _T("Long Lines"));
    AddColourListItem(hwnd, IDC_LIST1, TXC_CURRENTLINETEXT, TXC_CURRENTLINE,
                      _T("Current Line"));

    SendDlgItemMessage(hwnd, IDC_ITEMLIST, LB_SETCURSEL, 0, 0);
    PostMessage(hwnd, WM_COMMAND, MAKEWPARAM(IDC_ITEMLIST, LBN_SELCHANGE),
                (LPARAM)GetDlgItem(hwnd, IDC_ITEMLIST));

    for (i = 0; i < NUM_DEFAULT_COLOURS; i++) {
        AddColourComboItem(hwnd, IDC_FGCOLCOMBO, CUSTCOL[i].cr,
                           CUSTCOL[i].szName);
        AddColourComboItem(hwnd, IDC_BGCOLCOMBO, CUSTCOL[i].cr,
                           CUSTCOL[i].szName);
    }
    ...
    //	Select
    //
    wsprintf(ach, _T("%d"), g_nFontSize);

    SendDlgItemMessage(hwnd, IDC_SIZELIST, CB_SELECTSTRING, -1, (LPARAM)ach);
    SendDlgItemMessage(hwnd, IDC_FONTLIST, CB_SELECTSTRING, -1,
                       (LPARAM)g_szFontName);

    SetDlgItemInt(hwnd, IDC_PADDINGA, g_nPaddingAbove, 0);
    SetDlgItemInt(hwnd, IDC_PADDINGB, g_nPaddingBelow, 0);

    if ((g_fPaddingFlags & COURIERNEW) &&
        lstrcmpi(g_szFontName, _T("Courier New")) == 0) {
        SetDlgItemInt(hwnd, IDC_PADDINGA, g_nPaddingAbove, 0);
        SetDlgItemInt(hwnd, IDC_PADDINGB, g_nPaddingBelow, 1);
    }

    if ((g_fPaddingFlags & LUCIDACONS) &&
        lstrcmpi(g_szFontName, _T("Lucida Console")) == 0) {
...
INT_PTR CALLBACK AdvancedDlgProc(HWND hwnd, UINT msg, WPARAM wParam,
                                 LPARAM lParam)
{
    char lookup[] = {
        1, // DEFAULT_QUALITY
    ...
    case WM_INITDIALOG:
        ...
        AddComboStringWithData(hwnd, IDC_COMBO1, _T("None"),
                               NONANTIALIASED_QUALITY);
        AddComboStringWithData(hwnd, IDC_COMBO1, _T("Default"),
                               DEFAULT_QUALITY);
        AddComboStringWithData(hwnd, IDC_COMBO1, _T("Antialiased"),
                               ANTIALIASED_QUALITY);
        AddComboStringWithData(hwnd, IDC_COMBO1, _T("ClearType"),
                               CLEARTYPE_QUALITY);

        SendDlgItemMessage(hwnd, IDC_COMBO1, CB_SETCURSEL,
                           lookup[g_tempFontSmoothing], 0);
                                    ...
                                    LPARAM lParam)
{
    ...
    case WM_DRAWITEM:
        ...
        if (wParam == IDC_FONTLIST)
            ...
            return ColourCombo_DrawItem(hwnd, wParam, (DRAWITEMSTRUCT *)lParam,
                                        FALSE);
    ...
    case WM_NOTIFY:
        ...
        if (pshn->hdr.code == PSN_APPLY) {
            ...
            memcpy(g_rgbColourList, g_rgbTempColourList,
                   sizeof(COLORREF) * TXC_MAX_COLOURS);
    ...
    case WM_COMMAND:
        ...
        case IDC_ADVANCED:
            ...
            if (DialogBoxParam(GetModuleHandle(0),
                               MAKEINTRESOURCE(IDD_FONTEXTRA), hwnd,
                               AdvancedDlgProc, 0)) {
                UpdatePreviewPane(hwnd);
            }
        ...
        case IDC_BGCOLCOMBO:
            ...
            if (HIWORD(wParam) == CBN_SELCHANGE) {
                ...
                if (fgidx >= 0)
                    g_rgbTempColourList[fgidx] =
                        GetCurrentComboData(hwnd, IDC_FGCOLCOMBO);

                if (bgidx >= 0)
                    g_rgbTempColourList[bgidx] =
                        GetCurrentComboData(hwnd, IDC_BGCOLCOMBO);
