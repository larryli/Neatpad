...
HDC ShowPrintDlg(HWND hwndParent)
{
    ...
    if ((GetVersion() & 0xff) >= 5) {
        ...
        pdlgx.Flags = PD_RETURNDC | PD_NOPAGENUMS | PD_NOCURRENTPAGE |
                      PD_NOWARNING | PD_HIDEPRINTTOFILE;
        pdlgx.nStartPage = START_PAGE_GENERAL;

        if (S_OK == PrintDlgEx(&pdlgx) &&
            pdlgx.dwResultAction != PD_RESULT_CANCEL) {
...
int GetCurrentPrinterWidth(void)
{
