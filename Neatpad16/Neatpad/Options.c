...
void SetExplorerContextMenu(BOOL fAddToMenu)
{
    if (fAddToMenu) {
        ...
        RegSetValue(HKEY_CLASSES_ROOT, CONTEXT_CMD_LOC, REG_SZ, szDefaultStr,
...
void SetImageFileExecutionOptions(BOOL fReplaceWithCurrentApp)
{
    HKEY hKey;
    const TCHAR *szIFEO =
        _T("Software\\Microsoft\\Windows NT\\CurrentVersion\\Image File ")
        _T("Execution Options\\Notepad.exe");

    // create an 'ImageFileExecutionOptions' entry for the standard Notepad app
    if (S_OK == RegCreateKeyEx(HKEY_LOCAL_MACHINE, szIFEO, 0, 0, 0, KEY_WRITE,
                               0, &hKey, 0)) {
        ...
        // set the 'debugger' key so that whenever notepad.exe is executed,
        // neatpad runs instead
