...
HMENU TextView__CreateContextMenu(TEXTVIEW *ptv)
{
    ...
    AppendMenu(hMenu, MF_STRING | MF_DISABLED | MF_GRAYED, WM_USER + 0,
               _T("&Undo"));
    AppendMenu(hMenu, MF_STRING | MF_DISABLED | MF_GRAYED, WM_USER + 1,
               _T("&Redo"));
...
LONG TextView__OnContextMenu(TEXTVIEW *ptv, HWND hwndParam, int x, int y)
{
    ...
    UINT uCmd = TrackPopupMenu(hMenu, TPM_RETURNCMD, x, y, 0, ptv->m_hWnd, 0);

    if (uCmd == WM_COPY)
        PostMessage(ptv->m_hWnd, WM_COPY, 0, 0);

    // PostMessage(ptv->m_hWnd, WM_COMMAND, MAKEWPARAM(uCmd, 0),
    // (LPARAM)ptv->m_hWnd);
    return 0; // DefWindowProc(ptv->m_hWnd, WM_CONTEXTMENU, (WPARAM)hwndParam,
              // MAKELONG(x,y));
}
...
LONG TextView__OnLButtonDown(TEXTVIEW *ptv, UINT nFlags, int mx, int my)
{
    ...
    // mouse clicked within margin
        ...
        TextView__UpdateCaretOffset_outx_lineno(ptv, ptv->m_nCursorOffset,
                                                &ptv->m_nCaretPosX,
                                                &ptv->m_nCurrentLine);
        ptv->m_nAnchorPosX = ptv->m_nCaretPosX;
...
LONG TextView__OnMouseMove(TEXTVIEW *ptv, UINT nFlags, int mx, int my)
{
    if (ptv->m_nSelectionMode) {
        ...
        ptv->m_cpBlockEnd.xpos =
            ...
            TextView__LeftMarginWidth(ptv); // ptv->m_nCaretPosX;
        ...
        if (ptv->m_nSelectionMode == SEL_BLOCK)
            TextView__RefreshWindow(ptv);
        // ptv->m_nCaretPosX =
        // mx+ptv->m_nHScrollPos*ptv->m_nFontWidth-TextView__LeftMarginWidth(ptv);
...
VOID TextView__MoveCaret(TEXTVIEW *ptv, int x, int y)
{
    ...
    else if(x >= TextView__LeftMarginWidth(ptv) && ptv->m_fHideCaret == TRUE)
    {
...
{
    TextView__UpdateCaretOffset_outx_lineno(ptv, offset, NULL, NULL);
}

VOID TextView__UpdateCaretOffset_outx(TEXTVIEW *ptv, ULONG offset, int *outx)
{
    TextView__UpdateCaretOffset_outx_lineno(ptv, offset, outx, NULL);
}
...
ULONG TextView__RepositionCaret(TEXTVIEW *ptv, POINT *pt)
{
    ...
    // get line information from cursor-offset
    TextIterator itor = TextDocument__iterate_line_offset(ptv->m_pTextDoc,
ptv->m_nCursorOffset, &lineno, &off_chars);
