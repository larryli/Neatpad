...
void xSetStyle_bold(ATTR *attr, ULONG nPos, ULONG nLen, COLORREF fg,
                    COLORREF bg, int bold)
{
...
{
    xSetStyle_bold(attr, nPos, nLen, fg, bg, 0);
}
                           ...
                           ATTR *attr)
{
    ...
    while ((ptr2 = gettok(ptr1, tok, 128)) != 0) {
        if (isdigit(tok[0])) {
            xSetStyle_bold(attr, ptr1 - szText, ptr2 - ptr1, RGB(200, 0, 0),
                           RGB(255, 255, 255), 0);
        }
        if (memcmp(tok, _T("if"), 2) == 0 || memcmp(tok, _T("for"), 3) == 0) {
            xSetStyle_bold(attr, ptr1 - szText, ptr2 - ptr1, RGB(0, 0, 128),
                           RGB(255, 255, 255), 1);
        }
        ...
        if (lstrcmp(tok, _T("#include")) == 0) {
            // TextView__SetStyle(ptv, attr, ptr1 - szText, ptr2 - ptr1,
            // RGB(255,255,0), RGB(255,0,0));
            xSetStyle(attr, ptr1 - szText, ptr2 - ptr1, RGB(65, 102, 190),
                      RGB(255, 255, 255));
        } else if (lstrcmp(tok, _T("ULONG")) == 0) {
            xSetStyle(attr, ptr1 - szText, ptr2 - ptr1, RGB(0, 0, 255),
                      RGB(255, 255, 255));
