...
#ifdef UNICODE
    WCHAR *rawdata_w = (WCHAR *)(ptd->buffer + offset + ptd->headersize);
    WCHAR ch16;
    size_t ch32len = 1;
...
ULONG TextDocument__getdata(TEXTDOCUMENT *ptd, ULONG offset, BYTE *buf,
                            size_t len)
{
    memcpy(buf, ptd->buffer + offset + ptd->headersize, len);
    return len;
}
...
TEXTITERATOR *TextDocument__iterate(TEXTDOCUMENT *ptd, ULONG offset_chars)
{
    ...
    ULONG len_bytes = ptd->length_bytes - off_bytes;

    // if(!TextDocument__lineinfo_from_offset(ptd, offset_chars, 0, linelen,
    // &offset_bytes, &length_bytes)) 	return TextIterator__new();
