...
typedef int (*DRAW_GLYPH_PROC)(HDC hdc, int xpos, int ypos, RECT *rect,
                               int glyphIdx);
...
USPDATA *USP_Allocate(void);
void USP_Free(USPDATA *uspData);

BOOL USP_Analyze(USPDATA *uspData, HDC hdc, WCHAR *wstr, int wlen,
...
BOOL USP_SnapXToOffset(USPDATA *uspData, int mouseX,
                       int *snappedX, // out, optional
                       int *charPos,  // out
                       BOOL *fRTL     // out, optional
);

BOOL USP_XToOffset(USPDATA *uspData, int mouseX,
                   int *charPos,   // out
                   BOOL *trailing, // out
                   BOOL *fRTL      // out, optional
);

BOOL USP_OffsetToX(USPDATA *uspData, int offset, BOOL trailing,
                   int *xpos // out
);
...
void USP_SetSelColor(USPDATA *uspData, COLORREF fg, COLORREF bg);
