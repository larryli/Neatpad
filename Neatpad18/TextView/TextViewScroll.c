...
HRGN TextView__ScrollRgn(TEXTVIEW *ptv, int dx, int dy, bool fReturnUpdateRgn)
{
    ...
    if (dx != 0 || dy != 0) {
        ...
        ScrollWindowEx(ptv->m_hWnd,
                       ...
                       -dy * ptv->m_nLineHeight, NULL, // scroll entire window
...
VOID TextView__ScrollToPosition(TEXTVIEW *ptv, int xpos, ULONG lineno)
{
    bool fRefresh = false;
    RECT rect;
    int marginWidth = TextView__LeftMarginWidth(ptv);
    ...
    if (xpos < marginWidth) {
        ptv->m_nHScrollPos -= (marginWidth - xpos) / ptv->m_nFontWidth;
        fRefresh = true;
    }

    if (xpos >= rect.right) {
        ptv->m_nHScrollPos += (xpos - rect.right) / ptv->m_nFontWidth + 1;
        fRefresh = true;
    }

    if (lineno < ptv->m_nVScrollPos) {
        ptv->m_nVScrollPos = lineno;
        fRefresh = true;
    } else if (lineno > ptv->m_nVScrollPos + ptv->m_nWindowLines - 1) {
        ptv->m_nVScrollPos = lineno - ptv->m_nWindowLines + 1;
        fRefresh = true;
    }
