...
size_t utf8_to_utf32(UTF8 *utf8str, size_t utf8len, UTF32 *pch32)
{
    ...
    static UTF32 nonshortest[] = {0,       0x80,       0x800,
...
//                          on output, holds actual number of UTF16s stored in
//                          utf16str
//
//  Returns the number of bytes processed from utf8str
//
size_t utf8_to_utf16(UTF8 *utf8str, size_t utf8len, UTF16 *utf16str,
                     size_t *utf16len)
{
...
//  utf8len         - [in/out]  on input, specifies the size (in bytes) of
//  utf8str
//                              on output, holds actual number of bytes stored
//                              in utf8str
//
//  Returns the number of characters (UTF16s) processed from utf16str
//
size_t utf16_to_utf8(UTF16 *utf16str, size_t utf16len, UTF8 *utf8str,
                     size_t *utf8len)
{
    UTF16 *utf16start = utf16str;
    UTF8 *utf8start = utf8str;
...
//
size_t ascii_to_utf16(UTF8 *asciistr, size_t asciilen, UTF16 *utf16str,
                      size_t *utf16len)
{
    size_t len = min(*utf16len, asciilen);

    MultiByteToWideChar(CP_ACP, 0, (CCHAR *)asciistr, len, (WCHAR *)utf16str,
                        len);
    *utf16len = len;
    return len;
}
...
//
size_t utf16_to_ascii(UTF16 *utf16str, size_t utf16len, UTF8 *asciistr,
                      size_t *asciilen)
{
    size_t len = min(utf16len, *asciilen);
...
//  utf16str    - [out]     receives resulting utf-16 text
//  utf16len    - [in/out]  on input, specifies the size (in UTF16s) of utf16str
//                          on output, holds actual number of UTF16 values
//                          stored in utf16str
//
//  returns number of UTF32s processed from utf32str
//
size_t utf32_to_utf16(UTF32 *utf32str, size_t utf32len, UTF16 *utf16str,
                      size_t *utf16len)
{
    UTF16 *utf16start = utf16str;
...
//                          on output, holds actual number of UTF32 values
//                          stored in utf32str
//
//  returns number of UTF16s processed from utf16str
//
size_t utf16_to_utf32(UTF16 *utf16str, size_t utf16len, UTF32 *utf32str,
                      size_t *utf32len)
{
    UTF16 *utf16start = utf16str;
    UTF32 *utf32start = utf32str;
...
//  returns number of UTF16s processed from utf16str
//
size_t utf16be_to_utf32(UTF16 *utf16str, size_t utf16len, UTF32 *utf32str,
                        size_t *utf32len)
{
    UTF16 *utf16start = utf16str;
    UTF32 *utf32start = utf32str;
