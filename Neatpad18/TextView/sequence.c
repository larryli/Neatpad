...
//  Initialize from an on-disk file
//
// bool sequence__save(sequence * ps, TCHAR *filename)
//{
//  return false;
//}
...
bool sequence__clear(sequence *ps)
{
    ...
    for (size_t i = 0; i < ps->buffer_list.size; i++) {
        sequence__buffer_control *psbc =
            VECTOR_GET_AS(sequence__buffer_control *, &(ps->buffer_list), i);
        free(psbc->buffer);
        free(psbc);
    }
