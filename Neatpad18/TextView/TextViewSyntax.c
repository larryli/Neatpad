...
void xSetStyle(ATTR *attr, ULONG nPos, ULONG nLen, COLORREF fg, COLORREF bg,
               int bold)
{
                           ...
                           ATTR *attr)
{
    ...
    while ((ptr2 = gettok(ptr1, tok, 128)) != 0) {
        if (isdigit(tok[0])) {
            // xSetStyle(attr, ptr1 - szText, ptr2-ptr1, RGB(200,0,0),
            // RGB(255,255,255), 0);
            xSetStyle(attr, ptr1 - szText, ptr2 - ptr1, RGB(255, 80, 80),
                      RGB(255, 255, 255), 0);
        } else if (memcmp(tok, _T("if"), 2) == 0 ||
                   memcmp(tok, _T("for"), 3) == 0) {
            // xSetStyle(attr, ptr1 - szText, ptr2-ptr1, RGB(0,0,128),
            // RGB(255,255,255), 1);
            xSetStyle(attr, ptr1 - szText, ptr2 - ptr1, RGB(240, 240, 240),
                      RGB(255, 255, 255), 1);
        } else if (tok[0] == '\"' || tok[0] == '<' || tok[0] == '\'') {
            // xSetStyle(attr, ptr1 - szText, ptr2-ptr1, RGB(128,0,128),
            // RGB(255,255,255));
            xSetStyle(attr, ptr1 - szText, ptr2 - ptr1, RGB(200, 100, 200),
                      RGB(255, 255, 255), 0);
        } else if (tok[0] == '#') // lstrcmp(tok, _T("#include")) == 0)
        {
            // SetStyle(attr, ptr1 - szText, ptr2 - ptr1, RGB(255,255,0),
            // RGB(255,0,0)); xSetStyle(attr, ptr1 - szText, ptr2-ptr1,
            // RGB(65,102,190), RGB(255,255,255));
            xSetStyle(attr, ptr1 - szText, ptr2 - ptr1, RGB(100, 150, 200),
                      RGB(255, 255, 255), 0);
        } else if (lstrcmp(tok, _T("ULONG")) == 0) {
            xSetStyle(attr, ptr1 - szText, ptr2 - ptr1, RGB(100, 200, 255),
                      RGB(255, 255, 255), 0);
        } else if (lstrcmp(tok, _T("//")) == 0) {
            // xSetStyle(attr, ptr1 - szText, nTextLen - (ptr1-szText),
            // RGB(128,90,20), RGB(255,255,255));
            xSetStyle(attr, ptr1 - szText, nTextLen - (ptr1 - szText),
                      RGB(128, 110, 90), RGB(255, 255, 255), 0);
            break;
        } else {
            // xSetStyle(attr, ptr1 - szText, nTextLen - (ptr1-szText),
