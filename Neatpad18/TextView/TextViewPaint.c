...
USPDATA *TextView__GetUspData(TEXTVIEW *ptv, HDC hdc, ULONG nLineNo,
                              ULONG *nOffset /*=0 */)
{
    USPCACHE *uspCache = TextView__GetUspCache(ptv, hdc, nLineNo, nOffset);
...
LONG TextView__OnPaint(TEXTVIEW *ptv)
{
    ...
    for (i = first; i <= last; i++) {
        ...
        // TextView__PaintRect(ptv, hdcMem,
        // ptv->m_cpBlockStart.xpos+LeftMarginWidth(), 0,
        // ptv->m_cpBlockEnd.xpos-ptv->m_cpBlockStart.xpos,
        // ptv->m_nLineHeight,GetColour(TXC_HIGHLIGHT));
                         ...
                         int ypos, RECT *bounds)
{
    ...
    uspData = TextView__GetUspData(ptv, hdc, nLineNo, &lineOffset);
...
LONG TextView__OnNcPaint(TEXTVIEW *ptv, HRGN hrgnUpdate)
{
    ...
    if (ptv->m_hTheme != 0) {
        HDC hdc = GetWindowDC(
            ptv->m_hWnd); // GetDCEx(ptv->m_hWnd, GetWindowDC(ptv->m_hWnd);
        RECT rc;
        RECT rcWindow;
        DWORD state = ETS_NORMAL;

        if (!IsWindowEnabled(ptv->m_hWnd))
            state = ETS_DISABLED;
        else if (GetFocus() == ptv->m_hWnd)
            state = ETS_HOT;
        else
            state = ETS_NORMAL;

        GetWindowRect(ptv->m_hWnd, &rcWindow);
        GetClientRect(ptv->m_hWnd, &rc);
        ClientToScreen(ptv->m_hWnd, (POINT *)&rc.left);
        ClientToScreen(ptv->m_hWnd, (POINT *)&rc.right);
        ...
        // if (IsThemeBackgroundPartiallyTransparent (hTheme, EP_EDITTEXT,
        // state))
        //   DrawThemeParentBackground(ptv->m_hWnd, hdc, &rcWindow);

        DrawThemeBackground(ptv->m_hTheme, hdc, 6, state,
                            // EP_EDITTEXT,
                            // state,
                            // 3,0,
                            &rcWindow, NULL);
    ...
    return DefWindowProc(ptv->m_hWnd, WM_NCPAINT, (WPARAM)hrgnClip, 0);
}
