...
BOOL GetSettingInt(HKEY hkey, TCHAR szKeyName[], LONG *pnReturnVal,
                   LONG nDefault)
{
...
BOOL GetSettingStr(HKEY hkey, TCHAR szKeyName[], TCHAR pszReturnStr[],
                   DWORD nLength, TCHAR szDefault[])
{
...
{
    return !RegSetValueEx(hkey, szKeyName, 0, REG_BINARY, (BYTE *)pData,
                          nLength);
}
...
{
    return !RegSetValueEx(hkey, szKeyName, 0, REG_DWORD, (BYTE *)&nValue,
                          sizeof(nValue));
}
...
{
    return !RegSetValueEx(hkey, szKeyName, 0, REG_SZ, (BYTE *)szString,
...
BOOL SetExplorerContextMenu(BOOL fAddToMenu)
{
    ...
    if (fAddToMenu) {
        ...
        hr = RegSetValue(HKEY_CLASSES_ROOT, CONTEXT_CMD_LOC, REG_SZ,
                         szDefaultStr, lstrlen(szDefaultStr) * sizeof(TCHAR));
    } else {
...
//  manipulating the Image-File-Execution-Options debugger setting for
//  NOTEPAD.EXE
//
BOOL SetImageFileExecutionOptions(BOOL fReplaceWithCurrentApp)
{
    ...
    hr = RegCreateKeyEx(HKEY_LOCAL_MACHINE, IMAGEFILE_XOPT, 0, 0, 0, KEY_WRITE,
                        0, &hKey, 0);
    ...
    // set the 'debugger' key so that whenever notepad.exe is executed, neatpad
    // runs instead
...
void LoadRegSettings(void)
{
    ...
    GetSettingStr(hKey, _T("FontName"), g_szFontName, LF_FACESIZE,
                  _T("Courier New"));
    ...
    GetSettingInt(hKey, _T("PaddingFlags"), &g_fPaddingFlags,
                  COURIERNEW | LUCIDACONS);
    ...
    GetSettingInt(hColKey, _T("Foreground"), &g_rgbColourList[TXC_FOREGROUND],
                  g_rgbAutoColourList[TXC_FOREGROUND]);
    GetSettingInt(hColKey, _T("Background"), &g_rgbColourList[TXC_BACKGROUND],
                  g_rgbAutoColourList[TXC_BACKGROUND]);
    GetSettingInt(hColKey, _T("SelFG"), &g_rgbColourList[TXC_HIGHLIGHTTEXT],
                  g_rgbAutoColourList[TXC_HIGHLIGHTTEXT]);
    GetSettingInt(hColKey, _T("SelBG"), &g_rgbColourList[TXC_HIGHLIGHT],
                  g_rgbAutoColourList[TXC_HIGHLIGHT]);
    GetSettingInt(hColKey, _T("SelFG2"), &g_rgbColourList[TXC_HIGHLIGHTTEXT2],
                  g_rgbAutoColourList[TXC_HIGHLIGHTTEXT2]);
    GetSettingInt(hColKey, _T("SelBG2"), &g_rgbColourList[TXC_HIGHLIGHT2],
                  g_rgbAutoColourList[TXC_HIGHLIGHT2]);
    GetSettingInt(hColKey, _T("Margin1"), &g_rgbColourList[TXC_SELMARGIN1],
                  g_rgbAutoColourList[TXC_SELMARGIN1]);
    GetSettingInt(hColKey, _T("Margin2"), &g_rgbColourList[TXC_SELMARGIN2],
                  g_rgbAutoColourList[TXC_SELMARGIN2]);
    GetSettingInt(hColKey, _T("LinenoText"),
                  &g_rgbColourList[TXC_LINENUMBERTEXT],
                  g_rgbAutoColourList[TXC_LINENUMBERTEXT]);
    GetSettingInt(hColKey, _T("Lineno"), &g_rgbColourList[TXC_LINENUMBER],
                  g_rgbAutoColourList[TXC_LINENUMBER]);
    GetSettingInt(hColKey, _T("LongLineText"),
                  &g_rgbColourList[TXC_LONGLINETEXT],
                  g_rgbAutoColourList[TXC_LONGLINETEXT]);
    GetSettingInt(hColKey, _T("LongLine"), &g_rgbColourList[TXC_LONGLINE],
                  g_rgbAutoColourList[TXC_LONGLINE]);
    GetSettingInt(hColKey, _T("CurlineText"),
                  &g_rgbColourList[TXC_CURRENTLINETEXT],
                  g_rgbAutoColourList[TXC_CURRENTLINETEXT]);
    GetSettingInt(hColKey, _T("Curline"), &g_rgbColourList[TXC_CURRENTLINE],
                  g_rgbAutoColourList[TXC_CURRENTLINE]);

    GetSettingBin(hColKey, _T("Custom"), g_rgbCustColours,
                  sizeof(g_rgbCustColours));
...
void ApplyRegSettings(void)
{
    ...
    g_hFont = EasyCreateFont(g_nFontSize, g_fFontBold, g_nFontSmoothing,
                             g_szFontName);
    ...
    //  need to elevate using the UAC prompt. Only do this if the settings have
    //  actually changed
    //
