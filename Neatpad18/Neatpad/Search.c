...
void AddSearchTabs(HWND hwnd)
{
    ...
    g_hwndFindPane[0] = CreateDialog(
        g_hResourceModule, MAKEINTRESOURCE(IDD_FINDPANE), hwnd, FindHexDlg);
    g_hwndFindPane[1] = CreateDialog(
        g_hResourceModule, MAKEINTRESOURCE(IDD_REPLACEPANE), hwnd, FindHexDlg);
    g_hwndFindPane[2] = CreateDialog(
        g_hResourceModule, MAKEINTRESOURCE(IDD_GOTOPANE), hwnd, FindHexDlg);
    ...
    MoveWindow(hwndTab, FINDBORDER, FINDBORDER, rect.right - rect.left,
    ...
    SetWindowPos(hwnd, 0, 0, 0, rect.right - rect.left,
                 rect.bottom - rect.top - 2, SWP_SIZEONLY);
    ...
    for (i = 0; i < MAX_FIND_PANES; i++) {
        MoveWindow(g_hwndFindPane[i], rect.left, rect.top,
...
BOOL CALLBACK SearchDlgProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_NOTIFY:
        ...
        if (nmhdr->code == TCN_SELCHANGE) {
            ...
            SetWindowPos(g_hwndFindPane[idx], HWND_TOP, 0, 0, 0, 0,
...
HWND ShowFindDlg(HWND hwndParent, UINT nPage)
{
    ...
    if (g_hwndSearchDlg == 0) {
        g_hwndSearchDlg =
            CreateDialog(g_hResourceModule, MAKEINTRESOURCE(IDD_FIND),
                         hwndParent, SearchDlgProc);
...
void AddButton(HWND hwndTB, UINT uCmdId, UINT uImageIdx, UINT uStyle, TCHAR
*szText)
{
    //uStyle |= BTNS_SHOWTEXT;
    TBBUTTON tbb = { uImageIdx, uCmdId, TBSTATE_ENABLED, uStyle|BTNS_SHOWTEXT,
0, 0,0, (INT_PTR)szText };
...
HWND CreateEmptyToolbar(HWND hwndParent, int nBitmapIdx, int nBitmapWidth, int
nCtrlId, DWORD dwExtraStyle)
{
    HWND	   hwndTB;
    HIMAGELIST hImgList;
...
MAKEINTRESOURCE(nBitmapIdx), nBitmapWidth, 16, RGB(255,0,255), IMAGE_BITMAP,
LR_CREATEDIBSECTION);
...
LRESULT CALLBACK EditSubclassProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM
lParam)
{
    ...
    case WM_NCPAINT:
        ...
        DrawIconEx(hdc, rect.left, rect.top + (rect.bottom-rect.top-16)/2,
...
TOOLBARCLASSNAME,0,WS_VISIBLE|WS_CHILD|TBSTYLE_FLAT|CCS_NORESIZE|CCS_NODIVIDER,
            CreateEmptyToolbar(hwnd, IDB_BITMAP3, 18, 0, TBSTYLE_LIST);

        SendMessage(hwndTB, WM_SETFONT, (WPARAM)SendMessage(hwnd, WM_GETFONT, 0,
0), 0);

        MoveWindow(hwndTB, rect.right + 8, rect.top, 400, rect.bottom-rect.top,
TRUE);
...
TBSTYLE_EX_MIXEDBUTTONS|TBSTYLE_EX_DRAWDDARROWS );
        ...
        AddButton(hwndTB, 0, 3, TBSTYLE_BUTTON|TBSTYLE_CHECK, _T("Highlight
&all"));
...
HWND ShowSearchBar(HWND hwndParent)
{
    ...
    {
        g_hwndSearchBar = CreateDialog(g_hResourceModule,
MAKEINTRESOURCE(IDD_SEARCHBAR), hwndParent, SearchBarProc);
    }
