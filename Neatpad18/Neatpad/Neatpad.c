...
LRESULT CALLBACK AboutDlgProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_INITDIALOG:
        ...
        //  Set the dialog-icon
        //
        hIcon = (HICON)LoadImage(GetModuleHandle(0), MAKEINTRESOURCE(IDI_ICON1),
                                 IMAGE_ICON, 48, 48, 0);
        SendDlgItemMessage(hwnd, IDC_HEADER2, STM_SETIMAGE, IMAGE_ICON,
                           (WPARAM)hIcon);
        ...
        SendDlgItemMessage(hwnd, IDC_ABOUT_APPNAME, WM_SETFONT, (WPARAM)hFont,
                           0);
        ...
        hwndUrl = CreateWindow(WC_LINK, SYSLINK_STR,
                               WS_TABSTOP | WS_CHILD | WS_VISIBLE, rect.left,
                               rect.top, rect.right - rect.left,
                               rect.bottom - rect.top, hwnd, 0, 0, 0);
...
void ShowAboutDlg(HWND hwndParent)
{
    ...
    // DialogBoxWithFont(0, MAKEINTRESOURCE(IDD_ABOUT), hwndParent,
    // AboutDlgProc, 0, _T("MetaCondNormal-Roman"), 9);//_T("Bell MT Bold"));
}
...
UINT CommandHandler(HWND hwnd, UINT nCtrlId, UINT nCtrlCode, HWND hwndFrom)
{
    ...
    case IDM_FILE_SAVEAS:
        ...
        if (ShowSaveFileDlg(hwnd, g_szFileName, g_szFileTitle)) {
            MessageBox(hwnd, _T("Not implemented"), APP_TITLE,
                       MB_ICONINFORMATION);
        }
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_CREATE:
        ...
        TextView_SetContextMenu(
            g_hwndTextView,
            GetSubMenu(LoadMenu(GetModuleHandle(0), MAKEINTRESOURCE(IDR_MENU2)),
                       0));
    ...
    case WM_INITMENU:
        ...
        // CheckMenuCommand((HMENU)wParam, IDM_VIEW_SEARCHBAR, g_hwndSearchBar ?
        // TRUE : FALSE);

        EnableMenuCommand((HMENU)wParam, IDM_EDIT_UNDO,
                          TextView_CanUndo(g_hwndTextView));
        EnableMenuCommand((HMENU)wParam, IDM_EDIT_REDO,
                          TextView_CanRedo(g_hwndTextView));
        EnableMenuCommand((HMENU)wParam, IDM_EDIT_PASTE,
                          IsClipboardFormatAvailable(CF_TEXT));
        EnableMenuCommand((HMENU)wParam, IDM_EDIT_COPY,
                          TextView_GetSelSize(g_hwndTextView));
        EnableMenuCommand((HMENU)wParam, IDM_EDIT_CUT,
                          TextView_GetSelSize(g_hwndTextView));
        EnableMenuCommand((HMENU)wParam, IDM_EDIT_DELETE,
                          TextView_GetSelSize(g_hwndTextView));
    ...
    case WM_ENABLE:
        ...
        // keep the modeless find/replace dialog in the same enabled state as
        // the main window
    ...
    case WM_COMMAND:
        return CommandHandler(hwnd, LOWORD(wParam), HIWORD(wParam),
                              (HWND)lParam);
    ...
    case WM_CLOSE:
        ...
        if (TextView_CanUndo(g_hwndTextView)) {
            ...
            wsprintf(msgstr, _T("Do you want to save changes to\r\n%s?"),
                     g_szFileName);
            r = MessageBox(hwnd, msgstr, APP_TITLE,
    ...
    case WM_SIZE:
        ...
        // resize the TextView and StatusBar to fit within the main window's
        // client area
        ...
        if (g_fShowStatusbar) {
            DeferWindowPos(hdwp, g_hwndStatusbar, 0, 0, height - heightsb,
                           width, heightsb, SWP_SHOWWINDOW);
            //  MoveWindow(g_hwndStatusbar, 0, height - heightsb, width,
            //  heightsb, TRUE);
            height -= heightsb;
        }

        DeferWindowPos(hdwp, g_hwndTextView, 0, 0, 0, width, height,
                       SWP_SHOWWINDOW);
...
{
    return CreateWindowEx(0, g_szAppName,      // window class name
                   ...
                   int iShowCmd)
{
    ...
    //
    hAccel =
        LoadAccelerators(g_hResourceModule, MAKEINTRESOURCE(IDR_ACCELERATOR1));
    ...
    while (GetMessage(&msg, NULL, 0, 0) > 0) {
        if (!TranslateAccelerator(g_hwndMain, hAccel, &msg)) {
            if ((!IsWindow(g_hwndSearchDlg) ||
                 !IsDialogMessage(g_hwndSearchDlg, &msg))) {
