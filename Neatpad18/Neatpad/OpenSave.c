...
DWORD WINAPI ChangeNotifyThread(NOTIFY_DATA *pnd)
{
    ...
    do {
        ...
        if ((dwResult = WaitForMultipleObjects(1, hEventList, FALSE,
...
//
UINT_PTR CALLBACK OpenHookProc(HWND hwnd, UINT msg, WPARAM wParam,
                               LPARAM lParam)
{
    switch (msg) {
    case WM_INITDIALOG:
...
BOOL ShowOpenFileDlg(HWND hwnd, TCHAR *pstrFileName, TCHAR *pstrTitleName)
{
    ...
    ofn.Flags = OFN_EXPLORER | OFN_ENABLESIZING | OFN_ALLOWMULTISELECT |
                OFN_FILEMUSTEXIST | 0; // OFN_ENABLEHOOK          ;
...
//  used to center the dialog on 1st invokation and manage the 'encoding'
//  combobox
//
UINT_PTR CALLBACK SaveHookProc(HWND hwnd, UINT msg, WPARAM wParam,
                               LPARAM lParam)
{
    HWND hwndCombo;
    RECT rect;
    ...
    case WM_INITDIALOG:
        ...
        SendDlgItemMessage(hwnd, IDC_ENCODINGLIST, CB_ADDSTRING, 0,
                           (LPARAM) _T("Ascii"));
        SendDlgItemMessage(hwnd, IDC_ENCODINGLIST, CB_ADDSTRING, 0,
                           (LPARAM) _T("Unicode (UTF-8)"));
        SendDlgItemMessage(hwnd, IDC_ENCODINGLIST, CB_ADDSTRING, 0,
                           (LPARAM) _T("Unicode (UTF-16)"));
        SendDlgItemMessage(hwnd, IDC_ENCODINGLIST, CB_ADDSTRING, 0,
                           (LPARAM) _T("Unicode (UTF-16, Big Endian)"));
        SendDlgItemMessage(hwnd, IDC_ENCODINGLIST, CB_SETCURSEL, 0, 0);

        SendDlgItemMessage(hwnd, IDC_LINEFMTLIST, CB_ADDSTRING, 0,
                           (LPARAM) _T("Keep Existing"));
        SendDlgItemMessage(hwnd, IDC_LINEFMTLIST, CB_ADDSTRING, 0,
                           (LPARAM) _T("Windows (CR/LF)"));
        SendDlgItemMessage(hwnd, IDC_LINEFMTLIST, CB_ADDSTRING, 0,
                           (LPARAM) _T("Unix (LF)"));
        SendDlgItemMessage(hwnd, IDC_LINEFMTLIST, CB_ADDSTRING, 0,
                           (LPARAM) _T("Mac (CR)"));
        SendDlgItemMessage(hwnd, IDC_LINEFMTLIST, CB_ADDSTRING, 0,
                           (LPARAM) _T("Unicode (LT)"));
    ...
    case WM_SIZE:
        ...
        if (hwndCombo) {
            ...
            SetWindowPos(hwndCombo, 0, 0, 0, rect.right - rect.left,
                 ...
                 ...)
{
    ...
    _vstprintf(ptr, 1000, szFmt, varg);
...
{
    int fmt, fmtlook[] = {IDM_VIEW_ASCII, IDM_VIEW_UTF8, IDM_VIEW_UTF16,
                          IDM_VIEW_UTF16BE};
    ...
    if (TextView_OpenFile(g_hwndTextView, szFileName)) {
        ...
        CheckMenuRadioItem(GetMenu(hwndMain), IDM_VIEW_ASCII, IDM_VIEW_UTF16BE,
                           fmtlook[fmt], MF_BYCOMMAND);

        NotifyFileChange(szFileName, hwndMain, 0);
        return TRUE;
    } else {
        FmtErrorMsg(hwndMain, MB_OK | MB_ICONWARNING, GetLastError(),
                    _T("Error opening \'%ls\'\r\n\r\n"), szFileName);
        // FormatMessage
        // MessageBox(hwndMain, _T("Error opening file"), APP_TITLE,
        // MB_ICONEXCLAMATION);
        return FALSE;
    }
}
