...
)
{
    ...
    for (i = 0; i < runCount; i++) {
        ...
        if ((i == runCount - 1) ||
            xpos >= runx && xpos < runx + itemRun->width) {
            // get the character position
            ScriptXtoCP(xpos - runx, itemRun->len, itemRun->glyphCount,
                        uspData->clusterList + itemRun->charPos,
                        uspData->svaList + itemRun->glyphPos,
                        uspData->widthList + itemRun->glyphPos,
                        &itemRun->analysis, &cp, &trailing);

            // convert this character position back to a x-coord
            if (snappedX) {
                ScriptCPtoX(cp + trailing, FALSE, itemRun->len,
                            itemRun->glyphCount,
                            uspData->clusterList + itemRun->charPos,
                            uspData->svaList + itemRun->glyphPos,
                            uspData->widthList + itemRun->glyphPos,
                            &itemRun->analysis, snappedX);
    ...
    for (i = 0; i < runCount; i++) {
        ...
        if ((i == runCount - 1) ||
            ...
            ScriptXtoCP(xpos - runx, itemRun->len, itemRun->glyphCount,
                        ...
                        uspData->svaList + itemRun->glyphPos,
                        uspData->widthList + itemRun->glyphPos,
                        &itemRun->analysis, charPos, trailing);
...
//
BOOL WINAPI UspOffsetToX(USPDATA *uspData, int charPos, BOOL trailing, // out
                         int *px                                       // out
)
{
    int i;
    ...
    *px = 0;
    ...
    for (i = 0; i < uspData->itemRunCount; i++) {
        ...
        if ((i == uspData->itemRunCount - 1) ||
            charPos >= itemRun->charPos &&
                charPos < itemRun->charPos + itemRun->len) {
            ScriptCPtoX(
                charPos - itemRun->charPos, trailing, itemRun->len,
                itemRun->glyphCount, uspData->clusterList + itemRun->charPos,
                uspData->svaList + itemRun->glyphPos,
                uspData->widthList + itemRun->glyphPos, &itemRun->analysis, px);
