...
//  after this.
//
static BOOL ExpandTabs(USPDATA *uspData, WCHAR *wstr, int wlen,
                       SCRIPT_TABDEF *tabdef)
{
    ...
    int charWidth = uspData->uspFontList
                        ? uspData->uspFontList[0].tm.tmAveCharWidth
                        : uspData->defaultFont.tm.tmAveCharWidth;

    // validate the SCRIPT_TABDEF structure
    if (tabdef->cTabStops != 0 && tabdef->pTabStops == 0 ||
        tabdef->cTabStops < 0)
        return FALSE;
...
//
static int MergeSimpleScripts(SCRIPT_ITEM *itemList, int itemCount)
{
    ...
    for (i = 0; i < itemCount - 1; i++) {
        ...
        if (propList[itemList[i + 0].a.eScript]->fComplex == FALSE &&
            ...
            memmove(&itemList[i + 1], &itemList[i + 2],
                    (itemCount - i - 1) * sizeof(SCRIPT_ITEM));
            itemCount--;
...
//
//  Note that for scripts that do *not* have the fComplex property set,
//  (obtained via ScriptGetProperties) we can merge item-runs together to form a
//  single run with SCRIPT_UNDEFINED...this makes text-display *much* faster for
//  simple scripts such as english.
//
static BOOL BuildMergedItemRunList(USPDATA *uspData, WCHAR *wstr, int wlen,
                                   ATTR *attrList,
                                   ...
                                   SCRIPT_STATE *scriptState)
{
    ...
    do {
        ...
        if (uspData->tempItemAllocLen < allocLen) {
            ...
            // store this temporary-item list so we don't have to free/alloc all
            // the time
            uspData->tempItemAllocLen = allocLen;
            uspData->tempItemList = itemList;
        }

        hr = ScriptItemize(wstr, wlen, allocLen, scriptControl, scriptState,
                           itemList, &itemCount);
    ...
    while (i < itemCount) {
        ...
        // control-characters have their Unicode code-point stored in the
        // item-run (there will always be 1 ctrlchar per run)
        if (mergedList[m].ctrl)
            mergedList[m].chcode = wstr[mergePos];

        // safeguard against incorrect font usage when the user didn't specify a
        // font-list
        ...
        //
        // coalesce identical attribute cells into one contiguous run. Ignore
        // all style attributes except the ::font flags so even ATTR with
        // different colours will be merged together if they share the same
        // font.
        //
        // However, any ATTR with the ::ctrl  flag set will be isolated as a
        // single ITEM_RUN representing one control-character exactly.
        //
        if (attrLen == 0) {
            while (attrPos + attrLen < wlen) {
                ...
                if (attrPos + attrLen < wlen &&
                    (attrList[a].font != attrList[a + 1].font ||
                     ...
                     attrList[a].ctrl || attrList[a].eol))
                    break;
...
static BOOL BuildVisualMapping(ITEM_RUN *mergedRunList, int mergedRunCount,
                               BYTE bidiLevels[], int visualToLogicalList[])
{
    int i;
    ...
    ScriptLayout(mergedRunCount, bidiLevels, visualToLogicalList, 0);
...
//
static BOOL ShapeAndPlaceItemRun(USPDATA *uspData, ITEM_RUN *itemRun, HDC hdc,
                                 WCHAR *wrunstr)
{
    ABC abc;
    HRESULT hr;
    ...
    do {
        ...
        if (uspData->glyphCount + reallocSize >= uspData->glyphAllocLen) {
            ...
            uspData->glyphList = realloc(uspData->glyphList,
                                         uspData->glyphAllocLen * sizeof(WORD));
            uspData->offsetList = realloc(
                uspData->offsetList, uspData->glyphAllocLen * sizeof(GOFFSET));
            uspData->widthList = realloc(uspData->widthList,
                                         uspData->glyphAllocLen * sizeof(int));
            uspData->svaList =
                realloc(uspData->svaList,
                        uspData->glyphAllocLen * sizeof(SCRIPT_VISATTR));
        }
        //
        //  Convert the unicode-text into an array of glyphs
        //
        hr = ScriptShape(
            hdc, &uspFont->scriptCache, wrunstr, itemRun->len,
            uspData->glyphAllocLen - uspData->glyphCount, &itemRun->analysis,
            uspData->glyphList + itemRun->glyphPos,
            uspData->clusterList +
                itemRun->charPos, // already allocated in UspAnalyze
            uspData->svaList + itemRun->glyphPos, &itemRun->glyphCount);
    ...
    //
    ScriptPlace(hdc, &uspFont->scriptCache,
                uspData->glyphList + itemRun->glyphPos, itemRun->glyphCount,
                uspData->svaList + itemRun->glyphPos, &itemRun->analysis,
                uspData->widthList + itemRun->glyphPos,
                uspData->offsetList + itemRun->glyphPos, &abc);
...
//
static void IdentifyRunSelections(USPDATA *uspData, ITEM_RUN *itemRun)
{
    int c;
    int numsel = 0;
...
//
//  Any change to the fonts in uspFontList requires UspAnalyze to be called
//  again.
//
BOOL WINAPI UspAnalyze(USPDATA *uspData, HDC hdc, WCHAR *wstr, int wlen,
                       ATTR *attrRunList, UINT flags, USPFONT *uspFontList,
                       SCRIPT_CONTROL *scriptControl, SCRIPT_STATE *scriptState,
                       SCRIPT_TABDEF *scriptTabdef)
{
    ...
    //
    if (!BuildMergedItemRunList(uspData, wstr, wlen, attrRunList, scriptControl,
                                scriptState)) {
        return FALSE;
    }
    //  reallocate BIDI-arrays if item-run-list changed size
    if (itemRunAllocLen < uspData->itemRunAllocLen) {
        uspData->bidiLevels = realloc(uspData->bidiLevels,
                                      uspData->itemRunAllocLen * sizeof(BYTE));
        uspData->visualToLogicalList =
            realloc(uspData->visualToLogicalList,
                    uspData->itemRunAllocLen * sizeof(int));
    }
    //  Analyze the resulting runlist and build the BIDI-level array
    if (!BuildVisualMapping(uspData->itemRunList, uspData->itemRunCount,
                            uspData->bidiLevels,
                            uspData->visualToLogicalList)) {
        return FALSE;
    }
    ...
    if (uspData->stringAllocLen < wlen) {
        ...
        uspData->clusterList =
            realloc(uspData->clusterList, wlen * sizeof(WORD));
        uspData->attrList = realloc(uspData->attrList, wlen * sizeof(ATTR));
        uspData->breakList =
            realloc(uspData->breakList, wlen * sizeof(SCRIPT_LOGATTR));
    }
    // Generate the word-break information in logical order
    for (i = 0; i < uspData->itemRunCount; i++) {
        ITEM_RUN *itemRun = &uspData->itemRunList[i];

        ScriptBreak(wstr + itemRun->charPos, itemRun->len, &itemRun->analysis,
    ...
    for (i = 0; i < uspData->itemRunCount; i++) {
        ITEM_RUN *itemRun =
            GetItemRun(uspData, i); //;//&uspData->itemRunList[i];   //

        ShapeAndPlaceItemRun(uspData, itemRun, hdc, wstr + itemRun->charPos);
    }

    //
    // locate tab-characters and expand the corresponding glyph-widths
    // appropriately. this must happen after *all* shaping/widths have been
    // generated
    //
    if (scriptTabdef) {
...
//
USPDATA *WINAPI UspAllocate(void)
{
...
{
    if (uspData) {
        // free the script-cache (will be NULL if a user-supplied fontlist was
        // specified)
...
BOOL WINAPI UspBuildAttr(ATTR *attr, COLORREF colfg, COLORREF colbg, int len,
                         int font, int sel, int ctrl, int eol)
{
    if (attr == 0)
        return FALSE;
