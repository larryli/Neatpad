...
typedef struct _ATTR {
    ...
    int eol : 1;  // when set, prevents cursor from selecting character. only
                  // valid for last char in line
...
typedef struct _ITEM_RUN {
    ...
    int eol : 1; // prevents cursor from selecting character (only sensible for
                 // last char)
...
BOOL WINAPI UspAnalyze(USPDATA *uspData, HDC hdc, WCHAR *wstr, int wlen,
                       ATTR *attrRunList, UINT flags, USPFONT *uspFont,
...
int WINAPI UspTextOut(USPDATA *uspData, HDC hdc, int xpos, int ypos,
                      int lineHeight, int lineAdjustY, RECT *rect);

BOOL WINAPI UspSnapXToOffset(USPDATA *uspData, int xpos,
                             int *snappedX, // out, optional
                             int *charPos,  // out
                             BOOL *fRTL     // out, optional
);

BOOL WINAPI UspXToOffset(USPDATA *uspData, int xpos, int *charPos, // out
                         BOOL *trailing,                           // out
                         BOOL *fRTL // out, optional
);

BOOL WINAPI UspOffsetToX(USPDATA *uspData, int offset, BOOL trailing,
                         int *xpos // out
);

VOID WINAPI UspSetSelColor(USPDATA *uspData, COLORREF fg, COLORREF bg);

VOID WINAPI UspInitFont(USPFONT *uspFont, // in/out
                        HDC hdc,          // in
                        HFONT hFont       // in
);
