                                       ...
                                       ULONG *fileoff)
{
    ...
    if (lineno >= ptd->numlines || ptd->buffer == NULL || ptd->length == 0)
        return 0;
    ...
    if (fileoff)
        *fileoff = ptd->linebuffer[lineno]; // + offset;
                               ...
                               ULONG *length)
{
    if (lineno < ptd->numlines) {
        if (length)
            *length = ptd->linebuffer[lineno + 1] - ptd->linebuffer[lineno];
...
//
BOOL TextDocument__offset_to_line(TEXTDOCUMENT *ptd, ULONG fileoff,
                                  ULONG *lineno, ULONG *offset)
{
    ULONG low = 0, high = ptd->numlines - 1;
    ULONG line = 0;
    ...
    while (low <= high) {
        ...
        if (fileoff >= ptd->linebuffer[line] &&
            fileoff < ptd->linebuffer[line + 1]) {
            break;
        } else if (fileoff < ptd->linebuffer[line]) {
            high = line - 1;
        } else {
            low = line + 1;
        }
    ...
    if (lineno)
        *lineno = line;
    if (offset)
        *offset = fileoff - ptd->linebuffer[line];

    return TRUE;
}

ULONG TextDocument__size(TEXTDOCUMENT *ptd) { return ptd->length; }
