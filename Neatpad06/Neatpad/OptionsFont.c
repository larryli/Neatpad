...
static COLORREF g_rgbAutoColourList[TXC_MAX_COLOURS] = {
    SYSCOL(COLOR_WINDOWTEXT),          SYSCOL(COLOR_WINDOW),
    SYSCOL(COLOR_HIGHLIGHTTEXT),       SYSCOL(COLOR_HIGHLIGHT),
    SYSCOL(COLOR_INACTIVECAPTIONTEXT), SYSCOL(COLOR_INACTIVECAPTION),
};
...
BOOL InitFontOptionsDlg(HWND hwnd)
{
    ...
    oldPreviewProc =
        (WNDPROC)SetWindowLong(hwndPreview, GWL_WNDPROC, (LONG)PreviewWndProc);
